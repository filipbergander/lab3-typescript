...
export class Convert {
  ...
  convCelsiusToFahrenheit(): void {
    ...
    if (isNaN(Celsius)) { // Om användaren skrivit in något annat än tal, ex bokstav
      this.resultFahrenheit = 0;// Resultatet för Fahrenheit blir då 0
      return; // Kör inte vidare funktionen om användaren skriver en bokstav
    }
  ...
  convPoundToKilo(): void {
    ...
    const valueKilo = Pound * 0.4536; // Omvandling
    this.resultKilo = Number(valueKilo.toFixed(2));
  }

  convKiloToPound(): void {
    const Kilo = Number(this.weightKilo);
    if (isNaN(Kilo)) {
      this.resultPound = 0;
      return;
    }
    const valuePound = Kilo * 2.2046; // Omvandling
